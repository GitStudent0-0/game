...
public:
    ...
    RectangleShape drawLine(int idx1, int idx2)
    {
        ...
        RectangleShape line(sf::Vector2f(length, 10.f));
        line.setOrigin({ 0.f, 5.f });
        line.setFillColor(Color::Color(135, 163, 98));
        line.setPosition(p1);
    ...
    CircleShape drawTokens(const GameLogic& logic, int idxToken, bool toggle, float radius)
    {
        ...
        token.setPosition({ 600.f + x, 400.f + y });
        
        if (toggle)
            token.setFillColor(Color::Color(244, 247, 59));
        else
            token.setFillColor(Color::Color(166, 93, 212));
